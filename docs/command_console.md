...
reviewing operational events. It wraps the `CommandSender` helper in a
Flet-powered GUI while continuously tailing the `tspi.ops.status` heartbeat feed
to populate an operations log and the active client table.
...
2. **Group replay** – launch or stop datastore-backed group replays by entering
   the datastore identifier (or descriptive label) and optional display name for
   the shared channel. Operators can override the target stream when broadcasting
   to historical archives. The console tracks the active replay channel so the
   operator can stop it later or broadcast a replacement.
...
1. Enter the datastore identifier (or descriptive label) in the **Replay
   Identifier** field. This value becomes the shared channel label for clients.
2. Optionally provide a friendly **Display Name** to appear alongside the
   identifier on connected receivers.
...
4. Press **Start Replay** to broadcast the `GroupReplayStart` payload. Playback
